...
//========================================================================
// 函数: void EEPROM_SectorErase(uint32 EE_address)
// 描述: 擦除一个扇区.
...
void EEPROM_SectorErase(uint32 EE_address)
{
    ...
    IAP_ERASE();                        //宏调用, 送扇区擦除命令，命令不需改变时，不需重新送命令
                                        ...
                                        //扇区中任意一个字节地址都是扇区地址。
    IAP_ADDRE = (uint8)(EE_address >> 16); //送扇区地址高字节（地址需要改变时才需重新送地址）
    IAP_ADDRH = (uint8)(EE_address >> 8);  //送扇区地址中字节（地址需要改变时才需重新送地址）
    IAP_ADDRL = (uint8)EE_address;         //送扇区地址低字节（地址需要改变时才需重新送地址）
    EEPROM_Trig();                      //触发EEPROM操作
    DisableEEPROM();                    //禁止EEPROM操作
}

//========================================================================
// 函数: void EEPROM_read_n(uint32 EE_address,uint8 *DataAddress,uint8 lenth)
// 描述: 读N个字节函数.
...
void EEPROM_read_n(uint32 EE_address,uint8 *DataAddress,uint8 length)
{
    ...
    do
    {
        IAP_ADDRE = (uint8)(EE_address >> 16); //送地址高字节（地址需要改变时才需重新送地址）
        IAP_ADDRH = (uint8)(EE_address >> 8);  //送地址中字节（地址需要改变时才需重新送地址）
        IAP_ADDRL = (uint8)EE_address;         //送地址低字节（地址需要改变时才需重新送地址）
        EEPROM_Trig();                      //触发EEPROM操作
...
//========================================================================
// 函数: uint8 EEPROM_write_n(uint32 EE_address,uint8 *DataAddress,uint8 length)
// 描述: 写N个字节函数.
